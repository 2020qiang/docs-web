    ...
    markdown.index(index => {
        markdown.files(files => {
            ...
            for (let i = 0; i < index.length; i++) {
                if (index[i].hide) continue;
                add.link({title: index[i].title, href: index[i].route})
                add_line = true;
            }
            ...
            for (let i = 0; i < files.length; i++) {
                if (files[i] === "") continue;
                add.link({title: name.name(files[i]), href: browserUrl.float_split + files[i]});
            }
        });
    });
}
...
function active_outline() {
    ...
    for (let i = 0; i < contents.length; i++) {
        ...
        ele_piece.onmousedown = function () {
            ...
            if (ele.dataset.color_old === undefined) ele.dataset.color_old = ele.style.color;
            setTimeout(function () {
