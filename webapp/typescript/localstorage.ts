/* 时间戳小于15天,表示数据太老了 */
function is_old(key: string): boolean {
...
/* 是否存在缓存 */
function is_exist(key: string): boolean {
...
/* 写入新缓存 */
function write_new_cache(key: string, text: string): void {
    localStorage.setItem(key, text);
    localStorage.setItem(key + ".timestamp", Math.floor(Date.now() / 1000).toString());
}

function cache(key: string, fetch: Function): string {
    key = "cache." + key;
    if (!is_exist(key)) {
        const fetch_value: string | undefined = fetch();
        if (fetch_value === undefined) return "";
