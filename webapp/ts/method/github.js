 ...
 */
const auth = {
    user: config.json().auth.user,
    repo: config.json().auth.repo,
    branch: config.json().auth.branch,
 ...
 */
function list() {
    const text = cache.short("github.list", function () {
        /* 获取tree的sha */
...
function raw_http_url() {
    const markdown_path = decodeURI(raw_linux_path());
    return cache.short("markdown.url." + markdown_path, function () {
        /*
         * README.md
         */
...
function raw() {
     ...
     */
    return cache.short("markdown.raw." + decodeURI(raw_linux_path()), function () {
...
function index() {
    ...
    const text = cache.short("static", function () {
...
export { list, raw, raw_http_url, index, };
